...
- **Report progress.** Every time you finish a meaningful step (model done, endpoint working,
  tests passing), call `update_task` with a one-line `note`. The human operator watches the
  task board — a task with no notes looks stalled and may be reassigned.
...
---
**TL;DR loop:** list_tasks → claim_task → update_task in_progress → acquire_file_lock →
build (update_task note=... after each step) → (message on interface changes) →
release_file_lock → update_task done → repeat.
