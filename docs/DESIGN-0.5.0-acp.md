...
- 收到 `session/request_permission` → 0.5.1 起把 ACP 权限请求桥接到 0.4.0 审批门禁：写入类映射 `write`，命令类映射 `exec`，按 per-agent 策略 `allow / ask / deny` 处理；未知或只读请求保持自动放行。未声明 fs/terminal 能力（三个 agent 自带文件/执行能力，在 cwd 内自主操作）。
