...
- **agentic 活动呈现链（2026-06-15，Phase 0–1）**：让 stdio 直连的 CLI 真"动手并把过程显出来"。详见 [AGENTIC.md](./AGENTIC.md)。
  ...
  - **联机冒烟已做（2026-06-15）**：两解析器格式对真实 CLI 输出验证通过；发现 ① codex 用 `--sandbox workspace-write` 反把用户 config 的 full-access 降级、实际写不了（待 Codex 修，见 §6）② codex 多条 agent_message 会拼接（待 Codex 修）③ 测的 Claude-3p claude.exe 未登录。
...
- **codex agentic 写权限（待修，2026-06-15 联机冒烟发现）**：`adapters/codex.ts` 默认 `codex exec --json --sandbox workspace-write …`，
  实测 agent 自述"只读沙箱、写入被拦截"、文件没建成。根因疑似 `--sandbox workspace-write` 把用户 `~/.codex/config.toml`（本机为 `approval_policy="never"` + `sandbox_mode="danger-full-access"`）**往下降级**了。
