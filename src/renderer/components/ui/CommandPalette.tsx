...
export function CommandPalette({ open, onClose, commands, placeholder = 'Type a command or search…' }: CommandPaletteProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-[#1a1f2e]">
          <Search size={16} className="text-[#5c6478] shrink-0" />
          <input
            ref={inputRef}
            value={query}
            onChange={e => setQuery(e.target.value)}
            placeholder={placeholder}
            className="flex-1 bg-transparent text-sm text-[#e2e6ef] placeholder-[#3f4758] outline-none"
          />
          <button onClick={onClose} className="text-[#5c6478] hover:text-[#e2e6ef] p-1 rounded-md hover:bg-[#1a1f2e]">
            <X size={14} />
          </button>
        </div>
        ...
        <div ref={listRef} className="max-h-[55vh] overflow-y-auto p-2">
          {filtered.length === 0 ? (
            <div className="px-3 py-12 text-center text-[#5c6478] text-xs">
              No commands found for &ldquo;{query}&rdquo;
            </div>
          ) : (
            grouped.map(([group, items]) => (
              <div key={group} className="mb-1">
                <div className="px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-[#3f4758]">
                  {group}
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={[
                          ...
                          isActive ? 'bg-[#6366f1]/15 text-[#e2e6ef]' : 'text-[#a0a8ba] hover:bg-[#1a1f2e]',
                          c.disabled ? 'opacity-40 cursor-not-allowed' : ''
                        ].join(' ')}
                      >
                        {c.icon && <span className={isActive ? 'text-[#a5b4fc]' : 'text-[#5c6478]'}>{c.icon}</span>}
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-medium truncate">{c.label}</div>
                          {c.description && <div className="text-[10px] text-[#5c6478] truncate">{c.description}</div>}
                        </div>
                        ...
                        {isActive && !c.shortcut && <CornerDownLeft size={12} className="text-[#a5b4fc]" />}
                      </button>
                    )
                  })}
                </div>
        ...
        <div className="flex items-center gap-3 px-3 py-2 border-t border-[#1a1f2e] bg-[#0a0c12]/60 text-[10px] text-[#5c6478]">
