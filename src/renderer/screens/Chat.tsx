/* ============================================================
   AgentHub 玻璃拟态 UI — 会话 / 派发页
   智能路由 / 广播全部 / 链式接力 · 流式回复 · 思考折叠
   （流式数据由 App 监听 dispatch:stream 注入 messages）
   ============================================================ */
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 18, padding: '4px 2px' }}>
        {messages.length === 0 && (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, color: 'var(--tx-3)' }}>
            ...
            <div style={{ fontSize: 14 }}>{tr('输入任务开始派发 — 试试「重构 dispatcher 的轮询逻辑」', 'Type a task to dispatch — try "refactor the dispatcher polling logic"')}</div>
            <div className="ah-hint">{tr(`智能路由会按关键词选择 Agent；广播会同时询问全部 ${AGENT_IDS.length} 个`, `Auto route picks an agent by keywords; broadcast asks all ${AGENT_IDS.length} at once`)}</div>
          </div>
        )}
        {messages.map(m => (
  ...
  return (
    <Enter delay={delay} className="glass" style={{ padding: 14, display: 'flex', flexDirection: 'column', gap: 9, borderRadius: 16, minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
        ...
        {meta && <AgentMark id={r.agentId} size={24} radius={7} />}
        <span style={{ fontWeight: 600, fontSize: 13 }}>{meta?.name ?? r.agentId}</span>
        <span style={{ flex: 1 }}></span>
        {!r.done && r.text.length === 0 && r.thinking.length === 0 && <span className="ah-hint">{tr('连接中…', 'Connecting…')}</span>}
        {!r.done && (r.text.length > 0 || r.thinking.length > 0) && <span className="ah-dot busy"></span>}
        {r.cancelled && <span className="ah-hint">{tr('已停止', 'Stopped')}</span>}
        {r.done && !r.cancelled && !r.error && <Icon d={IC.check} size={14} style={{ color: 'var(--mint)' }} />}
      </div>
      {r.thinking && (
        <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: 10, overflow: 'hidden' }}>
          <button onClick={() => setThinkOpen(!thinkOpen)} style={{
            ...
            <Icon d={IC.brain} size={13} /> {tr('思考过程', 'Thinking')}
            <span style={{ flex: 1 }}></span>
